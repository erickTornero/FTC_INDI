...
import json
class Logger:
    def __init__(self, save_folder, namefile=None):
    ...
    def log(self, newtext, newline=True):
        print(newtext)
        newtext = newtext + ('\n' if newline else '')
        self.text += newtext
        

    def logwarn(self, newtext, newline=True):
        print(newtext)
        newtext = '[WARNING]' + newtext +('\n' if newline else '')
        self.text +=  newtext
        
    
    def logerr(self, newtext, newline=True):
        print(newtext)
        newtext =   '[ERROR]' + newtext + ('\n' if newline else '')
        self.text += newtext
        

    def load(self, namefile=None):
        namefile = self.namefile if namefile is None else self.namefile
        file_path           =   os.path.join(self.save_folder, namefile)
