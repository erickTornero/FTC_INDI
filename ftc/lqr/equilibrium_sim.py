...
class SolutionQuadrotorWrapper:
    ...
    def get_control_matrixes(self, failed_rotor: int, alpha_ratio: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def get_extended_control_matrixes(self, failed_rotor: int, alpha_ratio: float, up_time_motor: float):
        A, B = self.get_control_matrixes(failed_rotor, alpha_ratio)
        S = np.eye(2) * 1/up_time_motor
